...
def consolidate_intersections(
    ...
    """
    logs.info(
        f"consolidate_intersections start: tolerance={tolerance}, rebuild_graph={rebuild_graph}, dead_ends={dead_ends},"
        f" reconnect_edges={reconnect_edges}")
...
def _consolidate_intersections_rebuild_graph(G, tolerance=10, reconnect_edges=True):
    ...
    logs.info(f"_consolidate_intersections_rebuild_graph:\nSTEP 1: buffer nodes to passed-in distance and merge overlaps")
    ...
    # cluster it's within
    logs.info(f"_consolidate_intersections_rebuild_graph:\nSTEP 2: attach each node to its cluster of merged nodes")
    ...
    logs.info(f"_consolidate_intersections_rebuild_graph:\nSTEP 3: if a cluster contains multiple components move each component to its own cluster")
    ...
    logs.info(f"_consolidate_intersections_rebuild_graph:\nSTEP 4: create new empty graph and copy over misc graph data")
    H = nx.MultiDiGraph()
    H.graph = G.graph
    ...
    logs.info(f"_consolidate_intersections_rebuild_graph:\nSTEP 5: create a new node for each cluster of merged nodes")
    groups = gdf.groupby("cluster")
    for cluster_label, nodes_subset in groups:
        ...
        if len(osmids) == 1:
            ...
            H.add_node(
                ...
                x=nodes_subset["x"].mean(),
                y=nodes_subset["y"].mean(),
            )
    ...
    logs.info(f"_consolidate_intersections_rebuild_graph:\nSTEP 6: create new edge from cluster to cluster for each edge in original graph")
    ...
    logs.info(f"_consolidate_intersections_rebuild_graph:\nSTEP 7: for every group of merged nodes with more than 1 node in it")
    for cluster_label, nodes_subset in groups:
