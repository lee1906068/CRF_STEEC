...
def project_graph(G, to_crs=None):
    ...
    if "lng" not in gdf_nodes.columns or "y" not in gdf_nodes.columns:
        gdf_nodes["lng"] = gdf_nodes["x"]
    ...
    if "simplified" in G.graph and G.graph["simplified"]:
        ...
        gdf_edges_proj = utils_graph.graph_to_gdfs(G, nodes=False, fill_edge_geometry=True)
        gdf_edges_proj = project_gdf(gdf_edges_proj , to_crs=to_crs)
