...
def plot_graph(
    ...
    """
    max_node_size = max(node_size) if hasattr(
        node_size, "__iter__") else node_size
    max_edge_lw = max(edge_linewidth) if hasattr(
        edge_linewidth, "__iter__") else edge_linewidth
    ...
    if ax is None:
        fig, ax = plt.subplots(
            figsize=figsize, facecolor=bgcolor, frameon=False)
    ...
    if max_edge_lw > 0:
        ...
        ax = gdf_edges.plot(ax=ax, color=edge_color,
                            lw=edge_linewidth, alpha=edge_alpha, zorder=1)

    if max_node_size > 0:
        # scatter plot the nodes' x/y coordinates
        gdf_nodes = utils_graph.graph_to_gdfs(
            G, edges=False, node_geometry=False)[["x", "y"]]
        ax.scatter(
            x=gdf_nodes["x"],
            y=gdf_nodes["y"],
            s=node_size,
...
def plot_graph_route(
    ...
    **pg_kwargs,
    ...
    for u, v in zip(route[:-1], route[1:]):
        ...
        _length_min = 9999999
        _data_index = 0
        data = list(G.get_edge_data(u, v).values())
        for data_ in data:
            if data_['from'] == u and data_['to'] == v:
                _data.append(data_)
                if _length_min > data_['length']:
                    _length_min = data_['length']
                    _data_index = len(_data)-1

        data = _data[_data_index]
        ...
        for data_ in data:
            if data_['from'] == u and data_['to'] == v:
                ...
                if _length_min > data_['length']:
                    ...
                    _data_index = len(_data) - 1
        if len(_data) > 0:
            route_list.append(_data[_data_index])

    route_gdf = gpd.GeoDataFrame(pd.DataFrame(route_list), crs=G.graph["crs"])
    ax = route_gdf.plot(ax=ax, color=route_color,
                        lw=route_linewidth, alpha=route_alpha, zorder=1)
...
def plot_graph_route_gridmap(
    ...
    for u, v in zip(route[:-1], route[1:]):
        # if there are parallel edges, select the shortest in length
        # data = min(G.get_edge_data(u, v).values(), key=lambda d: d["length"])
        route_list.append(list(G.get_edge_data(u, v).values())[0])
    route_gdf = gpd.GeoDataFrame(pd.DataFrame(route_list), crs=G.graph["crs"])
    ax = route_gdf.plot(ax=ax, color=route_color,
                        lw=route_linewidth, alpha=route_alpha, zorder=1)
...
def plot_graph_routes(G, routes, route_colors="r", route_linewidths=4, **pgr_kwargs):
    ...
    if len(routes) != len(route_linewidths):  # pragma: no cover
        raise ValueError(
            "route_linewidths list must have same length as routes")

    # plot the graph and the first route
    override = {"route", "route_color",
                "route_linewidth", "show", "save", "close"}
...
def plot_graph_routes_gdf(G, routes, route_colors="r", route_linewidths=4, **pgr_kwargs):
    """
    Plot several routes along a graph.
...
def plot_graph_routes_gridmap(G, routes, route_colors="r", route_linewidths=4, **pgr_kwargs):
    """
    Plot several routes along a graph.
    ...
    override = {"route", "route_color",
                "route_linewidth", "show", "save", "close"}
    kwargs = {k: v for k, v in pgr_kwargs.items() if k not in override}
    fig, ax = plot_graph_route_gridmap(
        G,
        route=routes[0],
        route_color=route_colors[0],
        route_linewidth=route_linewidths[0],
...
def plot_figure_ground(
    ...
    **pg_kwargs,
    ...
    if G is not None:
        gdf_nodes = utils_graph.graph_to_gdfs(
            G, edges=False, node_geometry=True)
    ...
    for _, _, d in Gu.edges(keys=False, data=True):
        street_type = d["highway"][0] if isinstance(
            d["highway"], list) else d["highway"]
    ...
    if smooth_joints:
        ...
        for node in Gu.nodes:
            ...
            ie_data = [Gu.get_edge_data(node, nbr)
...
def _get_colors_by_value(vals, num_bins, cmap, start, stop, na_color, equal_size):
    ...
    if num_bins is None:
        ...
        color_list = [bin_colors[b] if pd.notnull(
            b) else na_color for b in bins]
...
def _save_and_show(fig, ax, save=False, show=True, close=True, filepath=None, dpi=300):
    ...
    if save:
        ...
        if ext == "svg":
            ...
            fig.savefig(filepath, bbox_inches=0, format=ext,
                        facecolor=fc, transparent=True)
        else:
