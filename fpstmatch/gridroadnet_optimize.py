...
class GridRoadNet:
    def __init__(self, analysisbounds=None, conn_info=None, config=None, loadexist=False, exisfile_root=None, _object=None, _object_pathset=None, grid_optimize=True):
        if not loadexist:
            self.conn_info = conn_info
            self.config = config
            self.analysisbounds = analysisbounds
            self.grid_optimize = grid_optimize
    ...
    def get_gridparams_optimize(self,ways_all):
        ways_all['lng'] = ways_all.apply(lambda x: [y[0] for y in x['geometry'].coords], axis=1)
        ways_all['lat'] = ways_all.apply(lambda x: [y[1] for y in x['geometry'].coords], axis=1)
        lngs = self.flatten(ways_all['lng'].tolist())
        lats = self.flatten(ways_all['lat'].tolist())
        ids =self.flatten([[row[1]['fid']]*len(row[1]['lng']) for row in ways_all[['lng','fid']].iterrows()])
        roadponints = pd.DataFrame({"Lng":lngs,"lat":lats,"id":ids}) 

        self.params_grid = tbd.grid_params_optimize(roadponints,self.params_grid,col=['id','Lng','lat'], optmethod='centerdist',sample=0)
        self.params_grid['gridsize'] = self.config['grid_accuracy']
    ...
    def get_nodes_ways_dets(self,
        ...
        if self.grid_optimize:
            self.get_gridparams_optimize(ways_all)
